...
// Função para exibir dados do usuário na tabela
function displayUserData(userData) {
  const tableBody = document.getElementById('userTable').getElementsByTagName('tbody')[0];
  const row = tableBody.insertRow();

  // Adiciona o nome e o email
  row.insertCell(0).textContent = userData.username || 'Nome não disponível';
  row.insertCell(1).textContent = userData.email || 'E-mail não disponível';

  // Verifica se o usuário possui trilhas
  const trilhas = userData.trilhas || {}; // Define trilhas como um objeto vazio se não existir
  const totalTrilhas = Object.keys(trilhas).length;

  // Se o usuário não iniciou nenhuma trilha, exibe uma mensagem
  if (totalTrilhas === 0) {
      row.insertCell(2).textContent = 'Nenhuma trilha iniciada';
      row.insertCell(3).textContent = '-'; // Exibe um traço na coluna de progresso das trilhas
      return; // Encerra a função para evitar erros de acesso a propriedades inexistentes
  }
